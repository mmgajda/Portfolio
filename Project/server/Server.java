...
public enum Server {
    ...
    private void start(int port) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(port);) {
            ...
            rooms.add(lobby);
            lobby.setMuteListHandler(muteListHandler);
            muteListHandler.setCurrentRoom(lobby);
            muteListHandler.loadMutedUserMap();
            do {
    ...
    void startQueueManager() {
        //Queue manager thread waits for the ServerThread thread to start
        //before officially handing them off to a room and opening them
        //for communication
        new Thread() {
            @Override
            public void run() {
                while (isRunning) {
                    ...
                    if (incomingClients.size() > 0) {
                        ...
                        if (ic != null) {
                            //wait for the thread to start and for the client
                            //to send the client name (username)
    ...
    protected synchronized boolean joinRoom(String roomName, ServerThread client) {
        ...
        if (newRoom != null) {
            if (oldRoom != null && oldRoom != newRoom) {
                logger.info(String.format("Client %s is leaving old room $s", client.getName(), oldRoom.getName()));
                oldRoom.removeClient(client);
                client.sendResetUserList();
            }
            logger.info(String.format("Client %s joining room %s ", client.getName(), newRoom.getName()));
    ...
    protected synchronized boolean createNewRoom(String roomName) {
        if (getRoom(roomName) != null) {
            ...
            //TODO: all non-lobby rooms will be gamerooms
            Room room = new Room(roomName); //chatroom projects can just use regular rooms
            room.setMuteListHandler(muteListHandler);
            muteListHandler.setCurrentRoom(room);
            rooms.add(room);
