...
if ($user_id > 0) {
    $db = getDB();
    $stmt = $db->prepare("SELECT name, c.id as line_id, item_id, quantity, unit_price, (unit_price*quantity) as subtotal FROM Shopping_Cart c JOIN Products i on c.item_id = i.id WHERE c.user_id = :uid");
    try {
        ...
        if ($balance >= $total_cost) {
            ...
            if ($next_order_id > 0) {
                ...
                set stock = stock - (select IFNULL(quantity, 0) FROM Shopping_Cart WHERE item_id = Products.id and user_id = :uid) 
                WHERE id in (SELECT item_id from Shopping_Cart where user_id = :uid)");
                try {
                    $stmt->execute([":uid" => $user_id]);
                } catch (PDOException $e) {
            ...
            if ($next_order_id > 0) {
                ...
                SELECT item_id, user_id, Shopping_Cart.quantity, unit_price, :order_id FROM Shopping_Cart JOIN Products on Shopping_Cart.item_id = Products.id
                WHERE user_id = :uid");
                try {
            ...
            //update inventory
            if ($next_order_id > 0) {
                $stmt = $db->prepare("INSERT INTO Inventory (item_id, quantity, user_id)
                SELECT item_id, quantity, user_id FROM Shopping_Cart WHERE user_id = :uid
                ON DUPLICATE KEY UPDATE Inventory.quantity = Inventory.quantity + Shopping_Cart.quantity");
                try {
                    $stmt->execute([":uid" => $user_id]);
                } catch (PDOException $e) {
            ...
            if ($next_order_id > 0) {
                $stmt =  $db->prepare("DELETE from Shopping_Cart where user_id = :uid");
                try {
                    $stmt->execute([":uid" => $user_id]);
                } catch (PDOException $e) {
            ...
            if ($next_order_id) {
                ...
                //deduct points
                give_tokens($total_cost, "purchase", get_user_account_id(), -1, "Spent $total_cost for items in the shop");
