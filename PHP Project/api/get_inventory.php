...
if (is_logged_in()) {
    ...
    /**item_id int,
    quantity int, */
    $stmt = $db->prepare("SELECT item_id, quantity, name, description FROM Inventory inv JOIN Products items on inv.item_id = items.id where inv.user_id = :uid and inv.quantity > 0");
    try {
